...
export function ScriptureReferenceChip({
  ...
  const href = getLocalColossiansReferenceHref(reference);
...
function getLocalColossiansReferenceHref(reference: string) {
  const match = reference.trim().match(/^Colossians\s+(\d+):(\d+)(?:[-–—]\d+)?$/iu);
  if (!match) return null;

  return `/colossians/${match[1]}/#v${match[2]}`;
}
